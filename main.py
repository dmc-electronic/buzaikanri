...
ZAIKO_HEADERS   = ["管理番号", "品番", "品名", "数量", "拠点", "保管場所", "更新日時"]
...
# ══════════════════════════════════════════════════
# 在庫 (ZAIKO) CRUD ヘルパー — 中古管理と同じロジック、"在庫"シート使用
# 列: 管理番号(A) 品番(B) 品名(C) 数量(D) 拠点(E) 保管場所(F) 更新日時(G)
# ══════════════════════════════════════════════════
def find_zaiko_row(ws: gspread.Worksheet, management_no: str) -> Optional[int]:
    col_a = ws.col_values(1)
    for i, val in enumerate(col_a[1:], start=2):
        if str(val).strip() == str(management_no).strip(): return i
    return None

def generate_zaiko_no(ws: gspread.Worksheet) -> str:
    col_a = ws.col_values(1)
    max_num = 0
    for val in col_a[1:]:
        m = re.match(r'^Z(\d+)$', str(val).strip())
        if m:
            n = int(m.group(1))
            if n > max_num: max_num = n
    return f"Z{max_num + 1:04d}"

def get_all_zaiko() -> List[Dict[str, Any]]:
    ...
    for row in rows[1:]:
        if not row or not row[0].strip(): continue
        result.append({
            "management_no": row[0].strip(),
            "part_no":       row[1].strip() if len(row)>1 else "",
            "name":          row[2].strip() if len(row)>2 else "",
            "qty":           int(row[3]) if len(row)>3 and str(row[3]).lstrip("-").isdigit() else 0,
            "base":          row[4].strip() if len(row)>4 else "",
            "location":      row[5].strip() if len(row)>5 else "",
...
def use_zaiko(management_no: str, qty: int, user: str) -> Dict[str, Any]:
    ws = open_sheet(WS_ZAIKO)
    row_no = find_zaiko_row(ws, management_no)
    if row_no is None: raise HTTPException(status_code=404, detail="対象の管理番号が見つかりません")
    row = ws.row_values(row_no)
    current_qty = int(row[3]) if len(row)>3 and str(row[3]).lstrip("-").isdigit() else 0
    new_qty = current_qty - qty
    if new_qty < 0: raise HTTPException(status_code=400, detail=f"在庫不足: 現在 {current_qty}, 使用 {qty}")
    now = jst_now()
    part_no = row[1] if len(row)>1 else ""
    name    = row[2] if len(row)>2 else ""
    base    = row[4] if len(row)>4 else ""
    location= row[5] if len(row)>5 else ""
    ws.update(f"D{row_no}:G{row_no}", [[str(new_qty), base, location, now]])
    append_history(user, part_no, f"{name} ({base} - {location})", qty, "zaiko_use")
    return {"management_no": management_no, "part_no": part_no, "name": name, "qty": new_qty, "base": base, "location": location, "updated_at": now}

def set_zaiko_fields(management_no: str, part_no: str, name: str, qty: int, base: str, location: str, user: str) -> Dict[str, Any]:
    ws = open_sheet(WS_ZAIKO)
    row_no = find_zaiko_row(ws, management_no)
    now = jst_now()
    if row_no:
        ws.update(f"B{row_no}:G{row_no}", [[part_no, name, str(qty), base, location, now]])
    else:
        raise HTTPException(status_code=404, detail="対象の管理番号が見つかりません")
    append_history(user, part_no, f"{name} ({base} - {location})", qty, "zaiko_edit")
    return {"management_no": management_no, "part_no": part_no, "name": name, "qty": qty, "base": base, "location": location, "updated_at": now}

def delete_zaiko_row(management_no: str) -> None:
    ws = open_sheet(WS_ZAIKO)
    row_no = find_zaiko_row(ws, management_no)
    if row_no is None: raise HTTPException(status_code=404, detail="対象の管理番号が見つかりません")
    ws.delete_rows(row_no)
...
class ZaikoUseRequest(BaseModel):
    management_no: str
    qty: int
    user: str = "ゲスト"

class AdminZaikoEditRequest(BaseModel):
    management_no: str
    part_no: str
    name: str
    qty: int
...
@app.post("/zaiko/use")
def use_zaiko_endpoint(body: ZaikoUseRequest):
    if body.qty < 1: raise HTTPException(status_code=400, detail="数量は1以上を入力してください")
    return use_zaiko(body.management_no, body.qty, body.user)
...
@app.put("/admin/zaiko/edit")
def admin_edit_zaiko(body: AdminZaikoEditRequest, _=Depends(get_admin_user)):
    if body.qty < 0: raise HTTPException(status_code=400, detail="数量は0以上を入力してください")
    return set_zaiko_fields(body.management_no, body.part_no, body.name, body.qty, body.base, body.location, body.user)

@app.delete("/admin/zaiko/{management_no}")
def admin_delete_zaiko(management_no: str, _=Depends(get_admin_user)):
    delete_zaiko_row(management_no); return {"message": "Deleted"}
